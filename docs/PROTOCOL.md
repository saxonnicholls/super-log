...
| `otlp.<service>`      | the OpenTelemetry inlet: OTLP/HTTP logs, metrics and spans on :4318 (JSON and protobuf), one topic per resource service.name; severityNumber maps to levels, trace_id becomes the event's `trace`, metrics become `metric` readings (histograms summarized), spans become trace-carrying DEBUG |
