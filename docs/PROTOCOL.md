...
| `ruby.<app>`          | a Ruby process (Rails via the `::Logger` adapter) |
| `perl.<app>`          | a Perl script (core modules only) |
| `lua.<app>`           | a Lua script (curl carries the bytes) |
| `csharp.<app>`        | a .NET process (C#, Unity profiles, Xbox Dev Mode) |
| `zig.<app>`           | a Zig program (the C SDK via `@cImport`) |
| `cobol.<app>`         | a GnuCOBOL program (the C SDK via a 20-line shim) |
...
| `git.<host>.<repo>`   | a checkout: commits, branch switches, rewritten history, tags, conflicts. `superlog-git` watches for change; `superlog git install-hooks` stamps a commit the instant it lands (same `change:commit` shape); `superlog git recall <sha>` replays the journal between a commit and the next |
...
| `otlp.<service>`      | the OpenTelemetry inlet: OTLP/HTTP logs, metrics and spans on :4318 (JSON and protobuf), one topic per resource service.name; severityNumber maps to levels, trace_id becomes the event's `trace`, metrics become `metric` readings (histograms summarized), spans become trace-carrying DEBUG. The reverse is `superlog-otlp-export` — bench → OTLP/JSON out to any backend (no topic; an outlet). See docs/OTLP.md |
| `starlink.dishy`      | the Starlink dish over its LAN gRPC API: pop latency/drop rate, up/down throughput, obstruction fraction and GPS sats as `metric` readings; alert flags, obstruction, firmware changes, reboots and unreachability as edge-triggered events with recovery |
| `rpc.<chain>`         | RPC node endpoint health: block height and latency per provider as `metric` readings; DOWN (ERROR) after two misses, STALLED (WARN) when the block stops advancing while still answering, recovery announced - run multiple providers per chain to see which failed |
| `gas.<chain>`         | operational key balances - EVM native/ERC-20, Solana native/SPL, Tron native/TRC-20, Bitcoin (Esplora): readings as `metric` events, edge-triggered CRITICAL below the fund-now line, WARN below the low line, recovery announced |
...
| `pr.<owner>.<repo>`   | pull requests watched by superlog-prs: one DEBUG row per PR per poll (`fields.waiting_on` us/them, `days_waiting` riding as a metric), edge-triggered WARN when a PR waits on us past the threshold, ERROR at 3x, recovery on reply; closed-without-merge is ERROR - that is what "closed as stale" looks like from outside |
